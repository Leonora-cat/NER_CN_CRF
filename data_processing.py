...
def extract_feature(tri_gram):
    # extract features of tri-grams
    """
    extract uni-gram and bi-gram inside tri-gram
    """
    tri_gram_feature = []
    for gram in tri_gram:
        feature = {'w-1': gram[0], 'w': gram[1], 'w+!': gram[2],
                   'w-1_w': gram[0] + gram[1], 'w_w+!': gram[1] + gram[2], 'bias': 1.0}
...
def get_model_input(word_seqs):
    ...
    for word_seq in word_seqs:
        ...
        tri_gram_feature = extract_feature(tri_gram)
        tri_grams_feature.append(tri_gram_feature)
    return tri_grams_feature


if __name__ == '__main__':
    corpus_post_process = data_processing(TRAIN_CORPUS_PATH)
